...
struct TokenizedString {
    ...
    init(_ string: String, using vocabulary: Vocabulary) {
        original = string
        
        let result = TokenizedString.tokenize(string, using: vocabulary)
    ...
    ///     - vocabulary: The vocabulary that is used to tokenize the string
    /// - returns: A tuple of an two arrays: one for tokens and another for their IDs.
    private static func tokenize(_ string: String, using vocabulary: Vocabulary) -> (tokens: [Substring], tokenIDs: [Int]) {
        let tokens = wordTokens(from: string)
        return wordpieceTokens(from: tokens, using: vocabulary)
    }
